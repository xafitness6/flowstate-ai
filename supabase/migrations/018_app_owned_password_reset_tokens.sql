-- App-owned password reset/setup codes.
--
-- These replace Supabase's one-time email action links for the critical
-- password form path. Email contains a short code, and the code is consumed
-- only when the user submits a new password.

CREATE TABLE IF NOT EXISTS public.auth_password_tokens (
  id          UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id     UUID        NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  email       TEXT        NOT NULL,
  token_hash  TEXT        NOT NULL UNIQUE, -- HMAC/hash of email + code
