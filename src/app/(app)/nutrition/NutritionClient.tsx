...
export type NutritionSSRData = {
  meals:     LoggedMeal[];
  hydration: number;
  targets:   NutritionTargets | null;  // null if no intake
} | null;

export default function NutritionClient({ initial }: { initial: NutritionSSRData }) {
  ...
  // Hydration (from store for the selected date) — seed from SSR
  const [hydration, setHydration] = useState(initial?.hydration ?? 0);

  // Skip the first useEffect refetch when SSR delivered today's data.
  // Subsequent date changes still trigger the normal client fetch.
  const initialDateRef = useRef(todayISO());
  const hasSSR = initial !== null;
  ...
  // ── Load targets ────────────────────────────────────────────────────────────
  // SSR populates targets for the initial paint; this effect refreshes them
  // (e.g. if intake was updated in another tab).
