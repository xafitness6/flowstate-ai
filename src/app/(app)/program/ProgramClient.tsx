...
export default function ProgramClient({ initial }: { initial: ProgramSSRData }) {
  const router  = useRouter();
  const { user, isLoading: userLoading } = useUser();

  const [program,    setProgram]    = useState<ActiveProgram | null>(initial?.program ?? null);
  const [weekLogs,   setWeekLogs]   = useState<WorkoutLog[]>(initial?.weekLogs ?? []);
  const [recentLogs, setRecentLogs] = useState<WorkoutLog[]>(initial?.recentLogs ?? []);
